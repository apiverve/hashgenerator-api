...
class HashgeneratorValidationException implements Exception {
  final List<String> errors;

  HashgeneratorValidationException(this.errors);

  @override
  String toString() => 'HashgeneratorValidationException: ${errors.join("; ")}';
}
...
/// Hash Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/hashgenerator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [text] (required) - Text to hash
/// * [algorithms] - Comma-separated list of algorithms or 'all'
class HashgeneratorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    ...
    'algorithms': ValidationRule(type: 'string', required: false),
  };

  HashgeneratorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/hashgenerator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [HashgeneratorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw HashgeneratorValidationException(errors);
    }
  }

  /// Execute a request to the Hash Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<HashgeneratorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw HashgeneratorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return HashgeneratorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw HashgeneratorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw HashgeneratorException('Resource not found');
      } else {
        throw HashgeneratorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is HashgeneratorException) rethrow;
      throw HashgeneratorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Hash Generator API client.
class HashgeneratorException implements Exception {
  final String message;

  HashgeneratorException(this.message);

  @override
  String toString() => 'HashgeneratorException: $message';
}
